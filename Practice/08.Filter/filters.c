...
int main(int argc, char* argv[])
{
    ...
    //Filter Design 선언
    //Gaussian Filter 5x5
    double Gaussian5F[5][5] = { {(double)1 / 273, (double)4 / 273, (double)7 / 273, (double)4 / 273, (double)1 / 273},
    ...
    //Gaussian Filter 3x3
    double Gaussian3F[3][3] = { {(double)1 / 16, (double)2 / 16, (double)1 / 16},
    ...
    for (int j = p; j < p + height; j++)
    {
        for (int i = p; i < p + width; i++)
        {
            ...
            for (int m = 0; m < 3; m++) //3x3 Filter (m,n) = (3,3)
                for (int n = 0; n < 3; n++)
                    F += result[(j - p + m) * pwidth + (i - p + n)] * Gaussian3F[m][n];
            result[j * pwidth + i] = F;
        }
    }
    ...
    //Gaussian 5x5 : 19.32dB
    //Mean 3x3 : 22.52dB
    //Gaussian 3x3 : 23.02dB

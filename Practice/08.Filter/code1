...
int main(int argc, char* argv[])
{
    ...
    for (int j = p; j < p + height; j++)
    {
        for (int i = p; i < p + width; i++)
        {
            ...
            for (int m = 0; m < 3; m++) //3x3 Filter
                for (int n = 0; n < 3; n++)
                    F += result[(j - p + m) * pwidth + (i - p + n)] * MeanF[m][n];
            result[j * pwidth + i] = F;
        }
    }
    ...
    printf("[originalY - testY]\nMSE = %.2lf(%.2lfdB)\n\n", mse, psnr);

    //원본 - 결과(노이즈를 필터 거친 결과) PSNR
    mse = 0;
    for (int j = 0; j < height; j++)
        for (int i = 0; i < width; i++)
            mse += (double)((result[(p + j) * pwidth + (p + i)] - y1[j * width + i]) * (result[(p + j) * pwidth + (p + i)] - y1[(p + j) * width + (p + i)]));
    mse /= (width * height);
    psnr = mse != 0.0 ? 10.0 * log10(255 * 255 / mse) : 99.99;
    printf("[originalY - output]\nMSE = %.2lf(%.2lfdB)\n\n", mse, psnr);

    //outputImg에 result 부분 할당
    for (int j = 0; j < height; j++)
    {
        for (int i = 0; i < width; i++)
        {

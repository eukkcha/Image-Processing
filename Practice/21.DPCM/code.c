...
int main(int argc, char* argv[])
{
	...
	// Function p: Predicted
	unsigned char* p = NULL;
	p = (unsigned char*)calloc(size1, sizeof(unsigned char));

	// Function dec: Reconstruction(decoder)
	unsigned char* dec = NULL;
	dec = (unsigned char*)calloc(size1, sizeof(unsigned char));

	int q = 5; // Quantization parameter
	int firstpixel = 128; // predefined value for the first pixel of each row
	for (int j = 0; j < height1; j++)
		for (int i = 0; i < width1; i++)
		{
			if (i == 0) p[j * width1 + i] = firstpixel;
			e[j * width1 + i] = o[j * width1 + i] - p[j * width1 + i];

			int value = (e[j * width1 + i] / q) * q;
			dec[j * width1 + i] == value;
		}
